...
func StartScheduler(linkProcessor *linkprocessing.LinkProcessor, errs chan error) error {
	...
	case "prod":
		...
		c.AddFunc("11 */3 * * *", async.WithContext(errs, "expire-accounts", makeExpireUserAccountsJob).Func())
	...
	case "local-test-emails",
		...
		c.AddFunc("*/3 * * * *", async.WithContext(errs, "expire-accounts", makeExpireUserAccountsJob).Func())
